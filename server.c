...
void login(FILE *db,int size){
    ...
    char tempo[8];
    int validUserNameOption,validUserPwdOption,returnOption,showAdminMenu=1,chk=0;
    char pass[30];
            ...
            if (strcmp(userData.username,userName)==0)
            {
                ...
                while (validUserPwdOption==0)
                {
                    ...
                    if (strcmp(userData.password,pass))
                    {
                        ...
                        clrscr();
                        fclose(db);
                        printf("\x1B[34mYou have successfully Logged In the System.\n");
    ...
    switch(choice){
        ...
        if(strcmp(curpasswd,userData.password)==0)
        {
            ...
            if(strcmp(newpasswd,confirmPassword))
            {
                ...
                printf("\x1B[31mYour new password is same as old password\n\x1B[0m");
            else{
                strcpy(user[i].password,newpasswd);
                printf("\x1B[34mYour password has been changed succesfully\n\x1B[0m");
            }
        }
        else{
        ...
        else printf("\x1B[31mInsufficient balance\n\x1B[0m");
        break;
        case 5:
        printf("\x1B[31mConfirm:Do you want to delete account permanantly?(Enter y to confirm)\n\x1B[0m");
        scanf(" %c",&confirm);
        if(confirm=='y'){
            while (validUserPwdOption==0)
            {
                ...
                {
                    printf("\x1B[31mYou Have Supplied a Wrong Password.\n\x1B[0m");
                    printf("\x1B[32mPlease re-enter your password: \n\x1B[0m");
                }
                else{
                    int temp=i;
                    clrscr();
                    ...
                    printf("%d\n",i);
                    printf("\x1B[34mYou can collect your balance amount Rs.%.2lf\n",userData.balance);
...
void createacc(FILE *db,int size){
    ...
    if(strcmp(password,confirmPassword))
    {
        ...
        if (checkSave=='y')
        {
            ...
            fflush(db);
            printf("----------------------\n");
            printf("\x1B[34mYour account has been created successfully and your Account number is %d\n\x1B[0m",userData.accno);
        } 
    }
    fclose(db);
}
